...
#let make-author(author, affls) = {
  ...
  return block(spacing: 0em, {
    ...
    show par: set par(spacing: 5.5pt)
    text(size: font-size.normal)[*#author.name*\ #author.email]
...
#let make-email(author) = {
  ...
  let label = text(size: font-size.small, (author.email))
  return block(spacing: 0em, {
...
#let jmlr(
  ...
  set page(
    paper: pageformat.paper,
    margin: (left: pageformat.left, right: pageformat.right, top: pageformat.top + 4.5pt, bottom: pageformat.bottom),
  ...
  if appendix != none {
    ...
    show heading: it => {
      ...
      block([#label.appendix~#numb~#it.body])
    }
